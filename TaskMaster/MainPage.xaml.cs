    ...
    public sealed partial class MainPage : Page
    {
        ...
        private string SaveNewProject(string projectName)
        {
            try
            {
                ...
                {
                    connection.Open();
                    string userQuery = "SELECT User_ID FROM [User] WHERE Username = @Username";
                    using (SqlCommand userCommand = new SqlCommand(userQuery, connection))
                    {
                        userCommand.Parameters.AddWithValue("@Username", UserSession.CurrentUsername);
